...
|---|---|
| CC6.1 (Logical access) | `designsystemharness-backend` exposes only its documented read/API endpoints - no unauthenticated write surface |
...
| CC7.1 (Change detection) | Branch = `designsystemharness_BE_nodejs22_FE_reactjs24_FE_angularjs19`, Node.js 22.x - version and provenance are pinned per branch |
...
This declaration covers the backend service in this branch (`designsystemharness_BE_nodejs22_FE_reactjs24_FE_angularjs19`). It is a repository-level control mapping, not an attestation issued by an auditor.
